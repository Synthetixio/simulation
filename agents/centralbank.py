    ...
    def step(self) -> None:
        ...
        if self.havven_target is not None:
            ...
            if havven_price > hm.round_decimal(self.havven_target * (Dec(1) + self.tolerance)):
                ...
                if self.havvens > 0:
                    self.place_havven_fiat_ask_with_fee(self._fraction(self.havvens),
                                                        self.havven_target)
                else:
                    ...
                    if available_havvens > 0:
                        self.unescrow_havvens(self._fraction(available_havvens))
                    ...
                    if self.unavailable_escrowed_havvens() > 0:
                        ...
                        if self.available_nomins > 0:
                            self.burn_nomins(self._fraction(self.available_nomins))
                        # Otherwise, we should buy some to burn, if we can.
                        elif self.available_fiat > 0:
                            self.sell_fiat_for_nomins_with_fee(self._fraction(self.available_fiat))
            ...
            # Price is too low, it should increase: we will buy havvens at a premium.
                ...
                if self.available_fiat > 0:
                    self.place_havven_fiat_bid_with_fee(self._fraction(self.available_fiat),
                                                        self.havven_target)
                else:
                    # If we have some nomins, sell them for fiat
                    if self.available_nomins > 0:
                        self.sell_nomins_for_fiat_with_fee(self._fraction(self.available_nomins))
                    else:
                        # If we have some havvens we could escrow to get nomins, escrow them.
                        if self.available_havvens > 0:
                            self.escrow_havvens(self._fraction(self.available_havvens))
        ...
        if self.nomin_target is not None:
            ...
            if nomin_price > hm.round_decimal(self.nomin_target * (Dec(1) + self.tolerance)):
                if self.available_nomins > 0:
                    self.place_nomin_fiat_ask_with_fee(self._fraction(self.available_nomins),
                                                       self.nomin_target)
                else:
                    ...
                    # Otherwise, obtain some.
                        ...
                        self.sell_fiat_for_havvens_with_fee(self._fraction(self.available_fiat))
            ...
            # Price is too low, it should increase: we will buy nomins at a premium.
                ...
                if self.available_fiat > 0:
                    self.place_nomin_fiat_bid_with_fee(self._fraction(self.available_fiat),
                                                       self.nomin_target)
                else:
                    if self.available_havvens > 0:
                        self.sell_havvens_for_fiat_with_fee(self._fraction(self.available_havvens))
                    else:
                        # If we do not have havvens, but we have some escrowed which we can
                        # immediately free, free them.
                        available_havvens = self.available_escrowed_havvens()
