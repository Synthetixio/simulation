...
def mean_escrownomshort_profit_fraction(havven_model: "model.HavvenModel") -> float:
    if len(havven_model.agent_manager.agents['CuritEscrowNominShorter']) == 0:
        return 0
    return mean(float(a.profit_fraction()) for a in havven_model.agent_manager.agents['CuritEscrowNominShorter'])
...
def fiat_demand(havven_model: "model.HavvenModel") -> float:
    ...
    curits = sum([float(ask.quantity * ask.price) for ask in havven_model.market_manager.curit_fiat_market.asks])
...
def fiat_supply(havven_model: "model.HavvenModel") -> float:
    ...
    curits = sum([float(bid.quantity * bid.price) for bid in havven_model.market_manager.curit_fiat_market.bids])
    nomins = sum([float(bid.quantity * bid.price) for bid in havven_model.market_manager.nomin_fiat_market.bids])
    return curits + nomins


def curit_demand(havven_model: "model.HavvenModel") -> float:
    """Return the total quantity of curits presently being bought in the marketplace."""
    nomins = sum([float(bid.quantity) for bid in havven_model.market_manager.curit_nomin_market.bids])
    fiat = sum([float(bid.quantity) for bid in havven_model.market_manager.curit_fiat_market.bids])
    return nomins + fiat


def curit_supply(havven_model: "model.HavvenModel") -> float:
    """Return the total quantity of curits presently being sold in the marketplace."""
    nomins = sum([float(ask.quantity) for ask in havven_model.market_manager.curit_fiat_market.asks])
    fiat = sum([float(ask.quantity) for ask in havven_model.market_manager.curit_nomin_market.asks])
    return nomins + fiat
...
def nomin_demand(havven_model: "model.HavvenModel") -> float:
    ...
    curits = sum([float(ask.quantity * ask.price) for ask in havven_model.market_manager.curit_nomin_market.asks])
...
def nomin_supply(havven_model: "model.HavvenModel") -> float:
    ...
    curits = sum([float(bid.quantity * bid.price) for bid in havven_model.market_manager.curit_nomin_market.bids])
