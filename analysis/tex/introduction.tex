...
We present Havven, a stablecoin system. It is an implementation of representative money seeking to achieve a stable price with respect to an external asset (such as the USD). Havven is a dual-token system that issues a stablecoin whose value is represented by the value of the cryptoeconomic system itself. In Havven, stablecoins are overcollateralised such that the value of the collateral pool is higher than the value of the stablecoins in circulation. Reserve tokens are placed in escrow for the purpose of issuing stablecoins as well as for earning fees generated from transactions.

A viable, autonomous, and decentralised stablecoin is a \textit{sine qua non} for achieving a decentralised economy.

Even with the powers afforded to central banks, there are many examples where currency stabilisation efforts have failed. Attempting to implement central banking mechanisms in a cryptoeconomic systems is even harder given that the behaviour of such systems must be completely defined at the start and they struggle to react to unforeseen market conditions.

On the other hand, while simpler, pure fiat-backed cryptocurrencies are necessarily subject to centralisation risks. 

Havven is thus a hybrid approach which simultaneously avoids many centralisation risks as well as the complexity of algorithmic central banking.

Backing a stablecoin by a cryptoeconomic asset is the most pragmatic approach to creating a stable cryptocurrency and accelerating the adoption of distributed ledger technology.
